...
/*: "USD" :*/
fileprivate let kStr_crownAlbumValue:String = "USexhibit"
...
class DecisionMakerBig: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = DecisionMakerBig()
    ...
    func observer() {
        ...
        let adjustConfig = ADJConfig(appToken: kLocalString, environment: environment)
        ...
        DecisionMakerBig.system(token: kSeaPowerValue)
    }
}
...
extension DecisionMakerBig: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func brain() -> String {
    ...
    class func present(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (kStr_crownAlbumValue.replacingOccurrences(of: "exhibit", with: "D")))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func agendaItem(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
