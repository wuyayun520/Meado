...
class HandicappedBackground {
    //: static let shared = AppPermissionTool()
    static let shared = HandicappedBackground()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func approval(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func establish(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func color(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
