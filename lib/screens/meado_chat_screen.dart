...
class MeadoChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const MeadoChatScreen({
    super.key,
  ...
  @override
  State<MeadoChatScreen> createState() => _MeadoChatScreenState();
}

class _MeadoChatScreenState extends State<MeadoChatScreen> {
  List<_MeadoChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/meado_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/meado_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _MeadoChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_MeadoChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _MeadoChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'meado_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_MeadoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _MeadoChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'meado_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_MeadoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _MeadoChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/meado_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _MeadoChatMessageType { text, image, audio }

class _MeadoChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _MeadoChatMessageType type;
  ...
  static _MeadoChatMessage fromJson(Map<String, dynamic> json) => _MeadoChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _MeadoChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _MeadoChatMessageType.text,
    ),
  );
}

class _MeadoChatBubble extends StatefulWidget {
  final _MeadoChatMessage message;
  final void Function(File file)? onImageTap;
  const _MeadoChatBubble({required this.message, this.onImageTap});

  @override
  State<_MeadoChatBubble> createState() => _MeadoChatBubbleState();
}

class _MeadoChatBubbleState extends State<_MeadoChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _MeadoChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/meado_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _MeadoChatMessageType.audio && msg.audioPath != null) {
    ...
    if (msg.type == _MeadoChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/meado_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/meado_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _MeadoChatInputBar extends StatelessWidget {
